...
class RadioGUI(wx.Dialog):
    ...
    def build_ui(self):
        ...
        stations = []
        for station in self.radio.config.stations:
            stations.append(f"{station.name}: {station.url}")
        ...
        self.stations = left_helper.addItem(wx.ListBox(self, wx.ID_ANY, choices=stations))
    ...
    def selection_station(self, event):
        ...
        self.radio.change_station(index)
    ...
    def add_station(self, event):
        ...
        if all([name, url]):
            new_position = self.radio.add_station(Station(name=name, url=url))
            self.new_station_name.SetValue('')
            self.new_station_url.SetValue('')
            self.add_station_button.Disable()
            station = self.radio.config.stations[new_position]
            self.stations.Insert(f"{station.name}: {station.url}", new_position)

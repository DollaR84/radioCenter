...
def find_lib():
    ...
    if sys.platform.startswith('win'):
        libname = 'libvlc.dll'
        p = os.path.join(os.path.dirname(__file__), "lib") # find_library(libname)
        if p is None:
            ...
            if dll is None and plugin_path is None:
            ...
            plugin_path = p
            dll = ctypes.CDLL(os.path.join(p, libname))
